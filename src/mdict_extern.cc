...
SizedData mdict_get_title(void *dict) {
  auto *self = (mdict::Mdict *)dict;
  const std::string s = self->get_title();
...
/**
 locate a word
 */
SizedData mdict_locate(void *dict, const char *word,
...
simple_key_item **mdict_search(void *dict, const char *query, int method,
  ...
  auto results = self->search(query ? query : "", method, max_results);
  ...
  *len = static_cast<uint64_t>(n);
  ...
  auto make_item = [](mdict::key_list_item *src) -> simple_key_item * {
    simple_key_item *item = new simple_key_item;
    item->record_start = src->record_start;
    item->key_word = strdup(src->key_word.c_str());
    return item;
  };

  for (std::size_t i = 0; i < n; ++i) {
    items[i] = make_item(results[i]);
  }
  return items;
}
...
simple_key_item **mdict_keylist(void *dict, uint64_t *len) {
  ...
  *len = static_cast<uint64_t>(n);
  ...
  // safe_strdup: always returns a pointer you may free() or nullptr on
  ...
  auto safe_strdup = [](const char *s, std::size_t len) -> char * {
    ...
    if (len >= SIZE_MAX / 2)
      len = 0; // paranoia guard
    std::size_t alloc_size = len + 1;
    char *p = static_cast<char *>(std::malloc(alloc_size));
    if (!p) {
      // Try to return an allocated marker string "<OOM>" (so caller can free
      // it)
  ...
  constexpr bool elem_is_ptr = std::is_pointer<Elem>::value;
...
// by atomic, we mean that each lookup is done isolated from each other,
