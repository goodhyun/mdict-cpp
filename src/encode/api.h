#ifndef MDICT_ENCODE_API_H
#define MDICT_ENCODE_API_H

#include "char_decoder.h"
#include <climits>
#include <cstdlib>
#include <stdlib.h>
#include <string.h>
#include <string>

// Safe casting wrapper
static inline unsigned char *malloc_uc(size_t size) {
  return reinterpret_cast<unsigned char *>(malloc(size));
}

// Adaptor for utf16le_to_utf8
static inline ssize_t utf16le_to_utf8_compat(const char *src, size_t src_len,
                                             char *dst, size_t dst_len) {
  return utf16le_to_utf8(reinterpret_cast<const unsigned char *>(src), src_len,
                         reinterpret_cast<unsigned char *>(dst), dst_len);
}

static inline char *hex_string_to_utf8(const char *hex_input) {
  if (!hex_input)
    return NULL;

  size_t hex_len = strlen(hex_input);
  unsigned char *utf16_buffer = NULL;
  char *utf8_output = NULL;

  // Handle empty input case
  if (hex_len == 0) {
    char *result = reinterpret_cast<char *>(malloc_uc(1));
    if (result)
      result[0] = '\0';
    return result;
  }

  // Convert hex to UTF-16LE bytes
  size_t utf16_buf_size = hex_len / 2;
  utf16_buffer = malloc_uc(utf16_buf_size);
  if (!utf16_buffer)
    return NULL;

  ssize_t utf16_bytes = hex_to_bytes(hex_input, utf16_buffer, utf16_buf_size);
  if (utf16_bytes < 0) {
    free(utf16_buffer);
    return NULL;
  }

  // Convert UTF-16LE to UTF-8
  size_t utf8_buf_size = (size_t)utf16_bytes * 3 + 1;
  utf8_output = reinterpret_cast<char *>(malloc_uc(utf8_buf_size));
  if (!utf8_output) {
    free(utf16_buffer);
    return NULL;
  }

  ssize_t utf8_bytes =
      utf16le_to_utf8_compat(reinterpret_cast<const char *>(utf16_buffer),
                             (size_t)utf16_bytes, utf8_output, utf8_buf_size);

  free(utf16_buffer);

  if (utf8_bytes < 0) {
    free(utf8_output);
    return NULL;
  }

  // Ensure null-termination
  if ((size_t)utf8_bytes < utf8_buf_size) {
    utf8_output[utf8_bytes] = '\0';
  } else {
    free(utf8_output);
    return NULL;
  }

  return utf8_output;
}

inline std::string trim_nulls(const std::string &s) {
  size_t end = s.find_last_not_of('\0');
  if (end == std::string::npos)
    return ""; // all nulls
  return s.substr(0, end + 1);
}

// meant to be used when checking dictionary headers
inline bool utf16_to_utf8_header(const unsigned char *src, size_t src_len,
                                 std::string &out) {
  if (!src)
    return false;
  if (src_len == 0) {
    out.clear();
    return true;
  }

  // worst-case: 4 bytes per UTF16 unit
  size_t out_capacity = src_len * 4;

  if (out_capacity > static_cast<size_t>(INT_MAX) ||
      src_len > static_cast<size_t>(INT_MAX)) {
    return false;
  }

  out.assign(out_capacity, '\0');

  ssize_t written = utf16le_to_utf8_compat(reinterpret_cast<const char *>(src),
                                           src_len, &out[0], out_capacity);

  if (written < 0 || static_cast<size_t>(written) > out_capacity) {
    out.clear();
    return false;
  }

  out.resize(static_cast<size_t>(written));
  return true;
}

#endif // MDICT_ENCODE_API_H
